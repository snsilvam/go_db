...
//PsqlInvoiceItem se usa para trabajar con postgre en InvoiceItemo
type PsqlInvoiceItem struct {
	db *sql.DB
}

//Retorna un nuevo pointerofpsqlInvoiceItem
func NewPsqlInvoiceItem(db *sql.DB) *PsqlInvoiceItem {
	return &PsqlInvoiceItem{db}
}

//Migrate implementa la interfaz InvoiceItem.storage
func (p *PsqlInvoiceItem) Migrate() error {
	...
	fmt.Println("Migración realizada del InvoiceItem")
	return nil
}
